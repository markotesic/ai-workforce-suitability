...
def custom_loader(dataset_path: str) -> Dataset:
    ...
    return MemoryDataset(samples=samples, name="abstract_narrative_understanding", location=dataset_path, shuffled=False)


@task
def abstract_narrative_understanding_task(
    ...
    annotation_csv_path = os.path.join(Path(__file__).parent, "abstract_narrative_understanding_annotations.csv")
