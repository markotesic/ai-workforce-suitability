...
def custom_loader(dataset_path: str) -> Dataset:
    ...
    return MemoryDataset(samples=samples, name="fantasy_reasoning", location=dataset_path, shuffled=False)


@task
def fantasy_reasoning_task(
    dataset_path: str | None = None,
    ) -> Task:
    if dataset_path is None:
        dataset_path = os.path.join(Path(__file__).parent, "fantasy_reasoning.json")
    dataset = custom_loader(dataset_path=dataset_path)

    # Filter dataset to only include annotated samples
    annotation_csv_path = os.path.join(Path(__file__).parent, "fantasy_reasoning_annotations.csv")
...
def annotate(num_samples: int = DEFAULT_NUM_SAMPLES, mode: str = "overwrite"):
    dataset_path = os.path.join(Path(__file__).parent, "fantasy_reasoning.json")
    output_path = os.path.join(Path(__file__).parent, "fantasy_reasoning_annotations.csv")
    dataset = custom_loader(dataset_path=dataset_path)
