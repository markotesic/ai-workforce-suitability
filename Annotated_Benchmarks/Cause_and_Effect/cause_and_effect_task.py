...
def record_to_sample(record: Dict[str, Any], dataset_path: str, id : int = 0) -> list[Sample]:
    ...
    sample = Sample(
        id=id,
        input= f"For each example, two events are given. Which event caused the other?: {record['input']}",
...
def custom_loader(dataset_path: str) -> Dataset:
    ...
    return MemoryDataset(samples=samples, name="cause_and_effect", location=dataset_path, shuffled=False)


@task
def cause_and_effect_task(
    dataset_path: str | None = None,
    ) -> Task:
    if dataset_path is None:
        dataset_path = os.path.join(Path(__file__).parent, "cause_and_effect.json")
    dataset = custom_loader(dataset_path=dataset_path)

    # Filter dataset to only include annotated samples
    annotation_csv_path = os.path.join(Path(__file__).parent, "cause_and_effect_annotations.csv")
...
def annotate(num_samples: int = DEFAULT_NUM_SAMPLES, mode: str = "overwrite"):
    dataset_path = os.path.join(Path(__file__).parent, "cause_and_effect.json")
    output_path = os.path.join(Path(__file__).parent, "cause_and_effect_annotations.csv")
    dataset = custom_loader(dataset_path=dataset_path)
